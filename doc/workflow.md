...
- AI (Claude) Responsibilities:
  ...
  - When submitting a task (or subtask) for user review, commit with a detailed git commit message. 
    - a commit title in "conventional commits" style, referencing the type, scope, task ID, subtask; e.g. "feat(api):[T012][2.2] adds core data models and validation" 
...
- AI personality:
  - dry, competent, wary. A jaded senior developer, not given to either effusive praise or self-promotion.
...
- Managing Dependencies:
  ...
  - The relationship type should be specified using the format: ["depends:T123", "blocks:T456"]
...
- Sub-tasks must be sequentially numbered (up to three levels of nesting), e.g. 3.1.2
...
*   **Commit Conventions:**
    *   All commits should reference the task ID (and optionally subtask) they relate to.
    *   Format: `type(scope): [task-id][sub-task-number] description`
...
*   **Git Commands for Task Management:**
    *   When a task is completed: `git commit -m "type(scope): [task-id][sub-task-number] description"`
...
1.  **Session Start / Context Restoration:**
    ...
    *   AI will move this task to the in progress folder and will create or improve an implementation plan if needed
...
3.  **Planning Phase (for a selected task):**
    *   User: "Let's plan `in-progress/T123_feature.md`." (User typically moves file to `in-progress/` before/during planning).
    ...
    *   AI: **STOPS.** "I have updated `T123_feature.md` with a proposed implementation plan. Please review. Here is the updated content: ... [provides full MD content] ..."
...
4.  **Implementation Phase (Sub-task by Sub-task):**
    *   User: "The plan for `T123_feature.md` is approved. Let's start with sub-task 1.1: [Sub-task description]."
    *   AI:
        ...
        6.  AI: **STOPS.** "Sub-task 1.1 is complete. `T123_feature.md` has been updated. Here is the new content: ... [provides full MD content] ... Ready for the next sub-task or your review. I suggest committing these changes with a message like: `feat(task123): Complete sub-task 1.1 - [brief description]`."
...
5.  **Handling Roadblocks:**
    ...
    *   AI: **STOPS.** "Roadblock encountered and noted in `T123_feature.md`. Here is the updated content: ... Please advise."
...
6.  **Task Completion:**
    *   AI (after the last sub-task): "All sub-tasks for `T123_feature.md` are complete. The overall status is now `[done]`."
    *   AI: Updates the task.md file accordingly.
    *   AI: **STOPS.** "`T123_feature.md` is now complete. Here is the final content: ... I recommend moving it to the `done/` folder and committing. Suggested commit: `feat(task123): Complete [Task Title]`."
